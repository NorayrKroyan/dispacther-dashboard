...
const filteredRows = computed(() => {
  ...
  return rows.sort((a, b) => {
    const aPriority = statusPriority[a.current_status || 'OFF DUTY'] ?? 99
    const bPriority = statusPriority[b.current_status || 'OFF DUTY'] ?? 99

    if (aPriority !== bPriority) {
      return aPriority - bPriority
    }

    const truckCompare = String(a.truck_number || '').localeCompare(String(b.truck_number || ''), undefined, {
      numeric: true,
      sensitivity: 'base',
    })
    ...
    return String(a.driver_name || '').localeCompare(String(b.driver_name || ''), undefined, {
      sensitivity: 'base',
    })
  })
})
...
function needsSectionGap(index) {
  ...
  return (previous?.current_status || 'OFF DUTY') !== (current?.current_status || 'OFF DUTY')
}
